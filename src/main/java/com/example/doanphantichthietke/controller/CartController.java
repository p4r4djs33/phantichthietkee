...
public class CartController {
    ...
    //SET TRẠNG THÁI GIỎ HÀNG THÀNH ĐÃ NHẬN
    @GetMapping("/home/cart/{id}/received")
    public ModelAndView received(@PathVariable Long id) {
        Optional<Cart> cart = cartService.findById(id);
        Cart cart1 = cart.get();
        cart1.setStatus("Đã nhận");
        cartService.save(cart1);
        ModelAndView modelAndView = new ModelAndView("redirect:/home/cart/{id}/vote");
        return modelAndView;
    }

    //SET TRẠNG THÁI GIỎ HÀNG THÀNH ĐÃ HỦY
    @GetMapping("/home/cart/{id}/cancel")
    public ModelAndView cancel(@PathVariable Long id) {
        Optional<Cart> cart = cartService.findById(id);
        Cart cart1 = cart.get();
        cart1.setStatus("Đã hủy đơn hàng");
        cartService.save(cart1);
        ModelAndView modelAndView = new ModelAndView("redirect:/home/cart/{id}/view/main");
        return modelAndView;
    }
    ...
    @GetMapping("/home/client/non-login")
    public ModelAndView nonLogin() {
        Cart cart = new Cart();
        LocalDateTime date = LocalDateTime.now();
        cart.setDateCreated(date);
        ModelAndView modelAndView = new ModelAndView("cart-non-login/createCart");
        modelAndView.addObject("cart", cart);
        return modelAndView;
    }
    ...
    @GetMapping("/home/cart/{id}/vote/save")
    public ModelAndView voteSave(@PathVariable Long id, Cart cart) {
        cartService.save(cart);
        ModelAndView modelAndView = new ModelAndView("redirect:/home/cart/{id}/view/main");
        return modelAndView;
    }
